...
class App:
    ...
    def accept_register_new_user(self):
        ...
        email = self.entry_text_register_new_user.get(1.0, "end-1c").strip()

        # Check if a file with this email already exists in the db directory
        if os.path.exists(os.path.join(self.db_dir, f'{email}.pickle')):
            self.msg_box('Error!', 'This email is already registered.')
            return

        # Get face embeddings from the captured image
        embeddings = face_recognition.face_encodings(self.register_new_user_capture)

        # Check if a face was detected
        if len(embeddings) > 0:
            embeddings = embeddings[0]
            # Save the embeddings in a pickle file named after the email
            with open(os.path.join(self.db_dir, f'{email}.pickle'), 'wb') as file:
                pickle.dump(embeddings, file)

            self.msg_box('Success!', 'User was registered successfully!')
            self.register_new_user_window.destroy()
        else:
            self.msg_box('Error!', 'No face detected. Please try again.')
